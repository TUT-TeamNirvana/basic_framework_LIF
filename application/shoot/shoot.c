...
// static DJIMotorInstance *friction_l, *friction_r, *loader; // 拨盘电机 // 暂禁用:摩擦轮/拨盘未安装,测云台/底盘用;恢复时取消本行注释及下方所有 //friction/loader 调用
...
void ShootInit()
{
    // 摩擦轮(暂禁用:摩擦轮未安装,测云台/底盘用;恢复时取消注释并恢复 friction_l/friction_r 初始化)
    ...
    // friction_l = DJIMotorInit(&friction_config);   // 暂禁用:摩擦轮未安装(测云台/底盘用)
    //
    // friction_config.can_init_config.tx_id = 1; // 右摩擦轮,改txid和方向就行
    // friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_NORMAL;
    // friction_r = DJIMotorInit(&friction_config);   // 暂禁用:摩擦轮未安装(测云台/底盘用)

    // 拨盘电机(暂禁用:拨盘未安装,测云台/底盘用;恢复时取消注释并恢复 loader 初始化)
...
void ShootTask()
{
    ...
    {
        // DJIMotorStop(friction_l);   // 暂禁用:摩擦轮未安装
        // DJIMotorStop(friction_r);
        //DJIMotorStop(loader);
    }
    else // 恢复运行
    {
        // DJIMotorEnable(friction_l); // 暂禁用:摩擦轮未安装
...
//    shoot_cmd_recv_vision_psy = shoot_cmd_recv.bullet_speed ;
    ...
    case BULLET_SPEED_NONE:
        // DJIMotorSetRef(friction_l, 0);   // 暂禁用:摩擦轮未安装
        ...
        break;
    case SMALL_AMU_15:
        // DJIMotorSetRef(friction_l, 31000);   // 暂禁用:摩擦轮未安装
        ...
        break;
    case SMALL_AMU_18:
        // DJIMotorSetRef(friction_l, 33000);   // 暂禁用:摩擦轮未安装
        ...
        break;
    case SMALL_AMU_30:
        // DJIMotorSetRef(friction_l, 35000);   // 暂禁用:摩擦轮未安装
