...
#include "ins_task.h"
#include "master_process.h" //包含视觉通信接口和视觉通信协议
...
void RobotCMDInit()
{
    ...
    vision_recv = VisionInit(NULL);  // VCP 模式传入 NULL
#else
    vision_recv = VisionInit(&huart1);  // UART 模式使用硬件串口
#endif
...
static void MouseKeySet()
{
    ...
    {
        // 使用新的视觉接收数据结构
        // 注意：需要确保 vision_recv 不为 NULL 且有数据
        if (vision_recv != NULL)
        {
            InputData_t *vision_input = &vision_recv->input_data;

            // 如果视觉有数据，使用视觉数据（默认鼠标右键是开启视觉模式）
...
//  视觉发送SendWait = 10 → 频率 ≈ 1000/10 = 100 Hz
void RobotCMDTask()
{
    // 从其他应用获取回传数据
#ifdef ONE_BOARD
